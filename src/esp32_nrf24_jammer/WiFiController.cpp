...
String WiFiController::generateHTML() {
    return String(
        ...
        "  const devices = [];"
        "  const startThreshold = 12;"
        "  const keepThreshold = 6;"
        "  const bridgeThreshold = 10;"
        "  const maxGap = 2;"
        "  let i = 0;"
        "  while (i < 126) {"
        "    if (channels[i] < startThreshold) {"
        "      i++;"
        "      continue;"
        "    }"
        "    let start = i;"
        ...
        "      const solidSignal = value >= keepThreshold;"
        "      const bridgedGap = gapCount < maxGap && value < keepThreshold && (i + 1) < 126 && channels[i + 1] >= bridgeThreshold;"
        "      if (!solidSignal && !bridgedGap) break;"
        "      if (solidSignal) {"
        ...
        "    let average = samples > 0 ? Math.round(sum / samples) : peak;"
        "    let strength = Math.max(0, Math.min(100, Math.round(((peak * 7) + (average * 3) + Math.min(width, 16)) / 10)));"
        "    let freqStart = 2400 + start;"
        "    let freqEnd = 2400 + end;"
        "    let type = 'RF Signal';"
        "    if (width >= 8 && width <= 28) type = 'WiFi';"
        "    else if (width <= 4) type = 'Bluetooth';"
        "    else if (width > 25) type = 'Wide Band';"
        "    devices.push({type, freqStart, freqEnd, strength});"
        "  }"
